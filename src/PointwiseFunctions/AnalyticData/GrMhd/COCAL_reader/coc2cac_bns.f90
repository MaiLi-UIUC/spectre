...
!______________________________________________
subroutine coc2cac_ir(npoints, x, y, z, lapse, shift_x, shift_y, shift_z, &
...
!
  ...
  !TODO remove this
  dir_path="/home/fs01/ml2847/SPECTRE/BNS/ID/IRE3.0_K123.6_K123.6_030_M2.8/work_area_BNS"
...
!  write(*,*) dir_path
  ...
  ! Read ID type
  filename= "/home/fs01/ml2847/SPECTRE/BNS/ID/IRE3.0_K123.6_K123.6_030_M2.8/work_area_BNS/rnspar_mpt1.dat"
...
!
 ...
 !   print *, "Interpolating Point:" , i
  ...
  !
        ...
        !   vyu = vepyfca / hca
        !   vzu = vepzfca / hca
        ! ! here is the lower indices velocities that we are using now
...
! now we compute velocity in the form that SpECTRE wants

! first we compute the lorentz factor

    ! lorentz_factor = SQRT(-1/(-1 + gxx * (- bvxdca / alphca + vxu) ** 2 
    !                       + gxy * (- bvxdca / alphca + vxu ) * (- bvydca / alphca + vyu )
    !                       + gxz * (- bvxdca / alphca + vxu ) * (- bvzdca / alphca + vzu )
    !                       + gyy * (- bvydca / alphca + vyu ) **2
    !                       + gyz * (- bvydca / alphca + vyu ) * (- bvzdca / alphca + vzu )
    !                       + gzz * (- bvzdca / alphca + vzu )**2))
                        
! then we compute the spatial velocities in the SpECTRE form
