...
!
  !TODO remove this
!  dir_path="/home/fs01/ml2847/SPECTRE/BNS/ID/IRE3.0_SLy_SLy_010_M1.96/work_area_BNS"
  !dir_path="../../standalone/Cocal/ID_BNS"
  !dir_path="."

!  write(*,*) "Reading initial data from..."
!  open(70,file='/home/fs01/ml2847/SPECTRE/src/PointwiseFunctions/AnalyticData/GrMhd/COCAL_reader/PATH2ID.txt',status='old')
!  read(70,'(i1)') iAB
...
!  if (ierr.ne.0)       write(6,'(a50)') "COCAL_ID:: problem reading file rnspar_mpt1.dat."

!  select case (id_type)
!    case ("CO")
!      write(6,*) "COCAL_ID:: Reading corotating BNS ID"
!      call coc2cac_co(dir_path(1:dir_path_len))
!    case ("IR")
!      write(6,*) "COCAL_ID:: Reading irrotational BNS ID"
!      call coc2cac_ir(dir_path(1:dir_path_len))
!    case ("SP")
!      write(6,*) "COCAL_ID:: Reading spinning BNS ID"
!      call coc2cac_sp(dir_path(1:dir_path_len))
!  end select
!
...
!END PROGRAM coc2cac_ini
!
!
!
subroutine cocal_interpolate(npoints, x, y, z, lapse, shift_x, shift_y, shift_z, &
                             ...
                             rest_mass_density, spatial_velocity_x, spatial_velocity_y, spatial_velocity_z) bind(C)
  use iso_c_binding  
...
!  use interface_excurve_CF_gridpoint_export
  ...
  ! Local variables
  integer(c_int) :: test_variable
  integer :: i
  real(8) :: psica, alphca, bvxdca, bvydca, bvzdca
  real(8) :: kxx, kxy, kxz, kyy, kyz, kzz
  real(8) :: rhoca, vxu, vyu, vzu
  real(8) :: gxx, gxy, gxz, gyy, gyz, gzz
  character(len=400) :: dir_path
  ...
  real(8) :: emdca, vepca, psi4ca, psif4ca
  real(8) :: hca, preca, eneca, epsca
  real(8) :: vepxfca, vepyfca, vepzfca, lam_p1, lam_p2
...
!  close(70)
  ...
  print *, "Reading initial data took:" , time_2 - time_1, "seconds"
...
!      call excurve_CF_gridpoint_export(alph_p3,bvxd_p3,bvyd_p3,bvzd_p3, axx_p3, axy_p3, axz_p3, ayy_p3, ayz_p3, azz_p3)
  ...
  print *, "Reading COCAL parameters took:" ,time_3 - time_2, "seconds" 
  ...
  ! Read ID type
  filename= "/home/fs01/ml2847/SPECTRE/BNS/ID/IRE3.0_SLy_SLy_010_M1.96/work_area_BNS/rnspar_mpt1.dat"
!  ierr = read_id_type(dir_path(1:dir_path_len)//'/'//'rnspar_mpt1.dat', id_type)
!  print *, "filename before the ieer function:", filename
  ierr = read_id_type(trim(filename), id_type)
!  print *, "filename after the ieer function:", filename
  if (ierr /= 0) then
  ...
  select case (id_type)
    ...
    case ("IR")
      write(6,*) "COCAL_ID:: Reading irrotational BNS ID"
!      Loop over each point to perform interpolation
      write(*,*) "Number of points:" ,npoints
      do i = 1, npoints
        ! Write coordinates to a temporary file
        call cpu_time(time_4)
        write(*,*) "Beginning writing input coordinates for point:", i
        open(unit=71, file='input_coords.txt', status='replace')
        write(71, *) x(i), y(i), z(i)
        close(71)

        ! Redirect input and call the original subroutine
        open(unit=5, file='input_coords.txt', status='old')
        write(6,*) "Coordinates written to input_coords.txt:", x(i), y(i), z(i)
        call cpu_time(time_5)
        print *, "Preparing coordinates took:" , time_5 - time_4, "seconds"
        call coc2cac_ir(dir_path(1:dir_path_len))
        close(5)
        call cpu_time(time_6)
        print *, "Interpolation took:" , time_6 - time_5, "seconds"

        open(unit=72, file='results_tmp.txt', status='old')
        read(72, *) psica, alphca, bvxdca, bvydca, bvzdca
        read(72, *) kxx, kxy, kxz, kyy, kyz, kzz
        read(72, *) rhoca, vxu, vyu, vzu
        read(72, *) gxx, gxy, gxz, gyy, gyz, gzz
        close(72)
        ...
        print *, "the lapse value is:" ,lapse(i)
        call cpu_time(time_7)
        print *, "Writing output took:" , time_7 - time_6, "seconds"      
      end do
...
end subroutine cocal_interpolate
...
!
  ...
  !dir_path="../../standalone/Cocal/ID_BNS"
  !dir_path='.'
  write(*,*) "In coc2cac_co: dir_path=",dir_path
...
!   write(6,'(e20.12)') dis_cm
!   call cpu_time(time_7)
!   print *, "Reading COCAL parameters took:" ,time_7 - time_4, "seconds" 
!
  write(6,'(a56)', ADVANCE = "NO") "Give cartesian coordinates (x,y,z) separated by a space:"
  read(5,*) xcac,ycac,zcac
  write(6,'(a23,3e20.12)') "Point given wrt CACTUS:", xcac,ycac,zcac
...
!  call cpu_time(time_8)
!  print *, "Interpolation in coc2cac_ir took:", time_8 - time_7, "seconds"
  gxx = psi4ca
  gxy = 0.0d0
  gxz = 0.0d0
  gyy = psi4ca
  ...
  open(unit=72, file='results_tmp.txt', status='replace')
  write(72, *) psica, alphca, bvxdca, bvydca, bvzdca
  write(72, *) kxx, kxy, kxz, kyy, kyz, kzz
  write(72, *) rhoca, vxu, vyu, vzu
  write(72, *) gxx, gxy, gxz, gyy, gyz, gzz
  close(72)
...
!
  ...
  !dir_path="../../standalone/Cocal/ID_BNS"
  !dir_path='.'
  write(*,*) "In coc2cac_sp: dir_path=",dir_path
